...
Beyond `go build`/`go test`, the backend has a black-box end-to-end suite in `server/e2e/`: **194 pytest tests** against the running HTTP stack (~3m36s cold; needs Docker, the Go toolchain and uv). The Go unit tests cover almost nothing, so this is the real gate for any change to auth, sync, integration or gateway *behaviour*:

```bash
cd server/e2e && uv venv && uv pip install -e .
.venv/bin/pytest                 # all 194; --keep-stack for faster reruns
```

It also includes hypothesis property tests and a stateful model of the sync protocol, and 28 `characterization`-marked tests pinning surprising behaviour. See `mem:server/e2e_tests`.
