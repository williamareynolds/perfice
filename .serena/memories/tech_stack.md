...
- One `server/Dockerfile` builds all four (`--build-arg SERVICE=<name>`, context `server/`). **Its builder stage takes no build args and compiles the whole workspace on purpose** — that makes it byte-identical across the four services so BuildKit builds the dependency tree once (~7 min) and the other three images take seconds. Building per-service (`cargo build -p perfice-auth`) makes each a separate cache entry, repeats the work 4x, and concurrently OOM-kills cargo.
- **The stack is one root `docker-compose.yml`** (2026-07-31): client + four services + MongoDB + RabbitMQ, all built from source. Config comes from a gitignored root `.env` (`.env.example` is the template, `just setup` fills it). Do *not* pull the `ghcr.io/p0lloc/perfice_*` images — those are upstream's Go build and expect Kafka.
