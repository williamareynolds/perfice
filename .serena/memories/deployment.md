...
- **Single origin via Caddy** (`caddy/Caddyfile`, `ORIGIN_PORT`, default 8080 — kanboard owns 80 here). The client and the gateway are behind one port, so nothing is cross-origin and **`CORS_EXTRA_ORIGINS` stays empty**. It previously had to match the client port exactly or the browser blocked every call while backend logs looked healthy.
  The Caddyfile enumerates the gateway's top-level prefixes (`/auth`, `/feedback`, `/api`, `/integrations`, `/integrationTypes`, `/updates`). **A prefix added to the gateway router and not here fails quietly** — the request falls through to the SPA and returns `index.html`, surfacing as a JSON parse error somewhere unrelated. `just smoke` runs through Caddy specifically to catch that.
  `servers { trusted_proxies … }` is a *global* option, not a site directive; putting it in the site block is an "unrecognized directive" crash loop.
